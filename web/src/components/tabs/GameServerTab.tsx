...
export function GameServerTab({ form, serverMode, gameStatus, onSwitchMode, onChange }: Props) {
  ...
  return (
    <TabPanel defaultValue="server">
      <TabPanel.Entry value="server" name="Server">
        <div className="grid max-w-2xl gap-3">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Route the game's socket traffic to the built-in server instead of the remote
                  Bondage Club server. Toggling reconnects the game immediately — you'll need to log
                  in again on the other server.
                </p>
              </div>
              <Switch
          ...
          {!IS_ANDROID_BUILD && (
            <Panel title="Storage">
              ...
              <p className="mt-2.5 text-xs text-muted-foreground">
                ...
                <code>{defaultDir}</code>. Saving a new location applies live — you'll be asked
                whether to migrate the existing accounts or start fresh; live game sockets reconnect
                either way.
              </p>
            </Panel>
          )}
          ...
          >
            ...
            <p className="mt-2.5 text-xs text-muted-foreground">
              ...
              choice is remembered in this browser; the server's default applies only on first
              launch.
            </p>
          </Panel>
        </div>
      </TabPanel.Entry>
      <TabPanel.Entry value="settings" name="Settings">
        <GameSettingsTab form={form} onChange={onChange} />
      </TabPanel.Entry>
    </TabPanel>
  );
}