...
// (The original BC server uses bcrypt; local accounts are created fresh, so no
// bcrypt compatibility is needed.) `iterations` is configurable; the iteration
// count is embedded in the output so existing hashes keep verifying when it
// changes.
